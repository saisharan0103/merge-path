...
> **Status:** Per `DECISIONS.md` this is a **single-user local app with no auth**.
> The `Authorization` header is not required and is ignored by the server.
> `POST /auth/login` is preserved as a no-op for spec compatibility.
...
- Errors are returned as `{ "detail": { "error": "<code>", "message": "<text>", "details": {...} } }`
  (FastAPI HTTPException default shape)
- Standard codes: 200, 201, 204, 400, 404, 409, 422, 429, 500
- Pagination: `?page=1&page_size=50` (default 20, max 100). Responses include
...
Returns the seeded user record.
...
No-op kept for spec compatibility. Returns
`{"access_token": "local", "token_type": "bearer", "user": {...}}`.
